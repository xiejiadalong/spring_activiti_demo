...
app.controller('userManergerController', function($rootScope, $scope, $http) {
	...
	$scope.onDelete = function(user_id) {
		...
		$http.post('/deleteUser', user).then(
	 ...
	 */
	var select_user_id;
	$scope.onSetRole = function(user_id) {
		select_user_id=user_id;
		$("#setUserRoleModal").modal("show");
		getAllRoles();
		setUserRoleData(user_id);
	}
    $scope.onAddRole= function() {
    	...
    	userRole.user_id = select_user_id;

		$http.post('/addUserRole', userRole).then(function successCallback(response) {
			setUserRoleData(select_user_id);
		}, function errorCallback(response) {
			console.log("error");
		});
	}
	
	$scope.onCancelForSetRole = function() {
		$("#setUserRoleModal").modal("hide");
	}
	
	function getAllRoles(){
		var parms = {
		};
		$http.post('/getAllRoles', parms).then(function successCallback(response) {
			var roledata=response.data;
			$("#slrole").html("");
			for(var i=0;i<roledata.length;i++){
				var role_id=roledata[i].role_id;
				var role_name=roledata[i].role_name;
				$("#slrole").append("<option value="+role_id+">"+role_name+"</option>");
			} 
		}, function errorCallback(response) {
			console.log("error");
		});
	}
	function setUserRoleData(user_id){
		var parms = {
		   "user_id":user_id
		};
		$http.post('/getUserRoles', parms).then(function successCallback(response) {
			var roledata=response.data;
			$scope.userRoleData=roledata;
		}, function errorCallback(response) {
			console.log("error");
		});
